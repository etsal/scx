 ...
 */
__weak
void scx_buddy_free_internal(struct scx_buddy *buddy, void __arena *addr)
{
	...
	if (addr & ((1ULL << 12) - 1)) {
		scx_bpf_error("Freeing non-page aligned address %p", addr);
		return;
	}
	...
	if (chunk == NULL) {
		bpf_spin_unlock(&buddy->lock);
		scx_bpf_error("could not find chunk for address %p", addr);
	}
