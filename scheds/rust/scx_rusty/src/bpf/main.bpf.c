...
static int scx_pick_idle_cpu(struct scx_cpumask __arena *scxmask, int flags)
{
	...
	cpu = scx_bpf_pick_idle_cpu(cast_mask(bpfmask), flags);
	scx_cpumask_from_bpf(scxmask, bpfmask);

	return cpu;
}

static int scx_pick_any_cpu(struct scx_cpumask __arena *scxmask, int flags)
{
	struct bpf_cpumask *bpfmask;
	int cpu = 0;
...
u64 tune_params_gen;
private(A) struct scx_cpumask all_cpumask;
private(A) struct scx_cpumask direct_greedy_cpumask;
private(A) struct scx_cpumask kick_greedy_cpumask;
...
static void refresh_tune_params(void)
{
	...
	bpf_for(cpu, 0, nr_cpu_ids) {
		...
		if (tune_input.kick_greedy_cpumask[cpu / 64] & (1LLU << (cpu % 64))) {
			scx_cpumask_set_cpu(cpu, &kick_greedy_cpumask);
		} else {
			scx_cpumask_clear_cpu(cpu, &kick_greedy_cpumask);
		}
	}
}
		   ...
		   u64 wake_flags)
{
	...
	prev_domestic = scx_cpumask_test_cpu(prev_cpu, taskc->cpumask);
	...
	if (prev_domestic) {
		 ...
		 */
		if (scx_cpumask_test_cpu(prev_cpu, &direct_greedy_cpumask) &&
	...
	if (has_idle_cores) {
		...
		cpu = scx_pick_idle_cpu(taskc->cpumask, SCX_PICK_IDLE_CORE);
		if (cpu >= 0) {
			stat_add(RUSTY_STAT_DIRECT_DISPATCH, 1);
			goto direct;
		}
	...
	cpu = scx_pick_idle_cpu(taskc->cpumask, 0);
	if (cpu >= 0) {
		stat_add(RUSTY_STAT_DIRECT_DISPATCH, 1);
		goto direct;
	}
	 ...
	 */
	if (taskc->all_cpus && !scx_cpumask_empty(&direct_greedy_cpumask)) {
		u32 dom_id = cpu_to_dom_id(prev_cpu);
		struct scx_cpumask tmp_direct_greedy;
		struct scx_cpumask node_mask;
		struct dom_ctx *domc;
		...
		scx_cpumask_copy(&tmp_direct_greedy, &direct_greedy_cpumask);
		...
		if (has_idle_cores) {
			...
			cpu = scx_pick_idle_cpu(&tmp_direct_greedy, SCX_PICK_IDLE_CORE);
			if (cpu >= 0) {
		...
		cpu = scx_pick_idle_cpu(&tmp_direct_greedy, 0);
		if (cpu >= 0) {
			stat_add(RUSTY_STAT_DIRECT_GREEDY_FAR, 1);
			goto direct;
		}
	...
	if (prev_domestic)
		...
		cpu = scx_pick_any_cpu(taskc->cpumask, 0);
...
dom_queue:
	 ...
	 */
	if (taskc->all_cpus) {
		cpu = scx_pick_idle_cpu(&kick_greedy_cpumask, 0);
		if (cpu >= 0) {
...
static s32 create_dom(u32 dom_id)
{
	struct dom_ctx *domc;
	struct node_ctx *nodec;
	struct scx_cpumask *dom_mask;
	struct bpf_cpumask *node_mask;
	u32 cpu, node_id;
	s32 ret;
	...
	bpf_for(cpu, 0, MAX_CPUS) {
		...
		if (*dmask & (1LLU << (cpu % 64))) {
			scx_cpumask_set_cpu(cpu, dom_mask);
	...
	scx_cpumask_from_bpf(dom_mask, node_mask);
	bpf_rcu_read_unlock();
...
static s32 initialize_cpu(s32 cpu)
{
	...
	bpf_for(i, 0, nr_doms) {
		...
		in_dom = scx_cpumask_test_cpu(cpu, domc->cpumask);
		bpf_rcu_read_unlock();
		if (in_dom) {
			pcpuc->dom_id = i;
			return 0;
