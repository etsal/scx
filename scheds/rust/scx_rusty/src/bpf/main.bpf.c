...
/*
 * Slight hack to help the verifier along. create_save_cpumask() makes a call
 * into the allocator code, adding significant complexity to the overall
 * function. Back-to-back allocations almost guarantee verification will fail
 * because of state space explosion.
 *
 * Add these __weak global wrappers around the allocations to have them verified
 * by themselves. Ideally we would make the allocation call itself __weak, but
 * the call returns a pointer which is not allowed by global functions.
 */
__weak int create_save_all_cpumask(void)
{
	return create_save_scxmask(&all_cpumask);
}

__weak int create_save_direct_greedy_cpumask(void)
{
	return create_save_scxmask(&direct_greedy_cpumask);
}

__weak int create_save_kick_greedy_cpumask(void)
{
	return create_save_scxmask(&kick_greedy_cpumask);
}
...
s32 BPF_STRUCT_OPS_SLEEPABLE(rusty_init)
{
	...
	ret = create_save_all_cpumask();
	if (ret)
		return ret;

	ret = create_save_direct_greedy_cpumask();
	if (ret)
		return ret;

	ret = create_save_kick_greedy_cpumask();
	if (ret)
		return ret;
