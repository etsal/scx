...
const volatile u32 runtime_threshold = 250 * 1000; /* 500 us */
...
void BPF_STRUCT_OPS(rusty_enqueue, struct task_struct *p __arg_trusted, u64 enq_flags)
{
	...
	if (taskc->dispatch_local) {
		taskc->dispatch_local = false;
		if (taskc->avg_runtime < runtime_threshold)
			scx_bpf_dsq_insert(p, SCX_DSQ_LOCAL_ON | taskc->domc->first_cpu, slice_ns, enq_flags);
		else
				     ...
				     bool init_dsq_vtime)
{
	...
	bpf_printk("Picked %d for %s", dom_id, p->comm);
