...
static inline bool refresh_layer_cpuc(struct cpu_ctx *cpuc, struct layer *layer)
{
	...
	if (cpuc->protected == layer->protected)
		false;

	cpuc->protected = layer->protected;
...
struct task_ctx {
	...
	struct cached_cpus	layered_unprotected;
...
static void maybe_refresh_layered_cpus_unprotected(struct task_struct *p, struct task_ctx *taskc)
{
	...
	if (should_refresh_cached_cpus(&taskc->layered_unprotected, 0, cpus_seq)) {
		refresh_cached_cpus(taskc->layered_unprotected_mask,
				    &taskc->layered_unprotected, 0, cpus_seq,
				    p->cpus_ptr,
				    cast_mask(unprotected_cpumask));
		trace("%s[%d] layered allowed cpumask refreshed to seq=%llu",
		      p->comm, p->pid, taskc->layered_unprotected.seq);
	}
}
...
void BPF_STRUCT_OPS(layered_dispatch, s32 cpu, struct task_struct *prev)
{
	 ...
	 */
	if (!cpuc->protected && scx_bpf_dsq_nr_queued(cpuc->lo_fb_dsq_id)) {
		u64 now = scx_bpf_now();
		u64 dur, usage;
	...
	if (cpuc->in_open_layers) {
		 ...
		 */
		if (!owner_layer || (!owner_layer->protected && !cpuc->protect_owned && !owner_layer->preempt)) {
		   ...
		   struct scx_init_task_args *args)
{
	...
	ret = init_cached_cpus(&taskc->layered_unprotected);
	if (ret)
		return ret;
